...
class JwtTokenProvider(
     ...
     */
    fun createAccessToken(userId: String, userType: UserType?, email: String): String {
        val now = Date()
        val validity = Date(now.time + accessTokenValidityInSeconds * 1000)

        return Jwts.builder()
            .setSubject(userId)
            .claim("userType", userType?.code)
            .claim("email", email)
    ...
    fun getAuthentication(token: String): Authentication {
        ...
        userType?.let { authorities.add(SimpleGrantedAuthority("ROLE_$it")) }

        val principal = UserPrincipal(
            userId = userId,
            email = email ?: "",
            userType = userType?.let { UserType.fromCode(it) }
        )
